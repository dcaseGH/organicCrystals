...
class AnalyseClusters(unittest.TestCase):
    ...
    def test_rdkitIO(self):
        ...
        np.testing.assert_array_almost_equal(np.array([x.position for x in struct]),
                                             ...
                                             decimal=2)
        #do again but with option to reassign positions
...
#        structs = ASERead('CSPPCM_example.cif', index=':') #old way - dont do this
        ...
        splitStructure = splitASEAtomsToMols(structures[-2], useOriginalPositions = False)
        np.testing.assert_array_almost_equal(np.array([x.position for x in structures[-2]]),
        ...
        #still havent sorted the pdb stuff (as in my own formatting - so use the flag above to get positions)
...
class TestCrystal(unittest.TestCase):
    ...
    def test_readingCIFs(self):
        ...
        self.assertTrue(len(myCrystal.asymmetricMolecules), 
                        2)
        self.assertEqual([len(x.aseAtoms) for x in myCrystal.asymmetricMolecules],
                         [30, 30])
